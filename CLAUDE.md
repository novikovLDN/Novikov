...
раскодируется из шума при появлении), `SpotlightLayer` (световое
пятно за курсором на любой карточке с классом `.px-spot` — один
делегированный слушатель на документ, а не ref в каждом компоненте),
`useTilt` (наклон плоскости за курсором), `RollingNumber` (барабан
разрядов). Приёмы взяты из практики наградных промо-сайтов и
