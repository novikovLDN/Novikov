...
**Два адресата витрины.** Сайт говорит и с частным лицом, и с
компанией. Развилка сделана содержанием, а не вторым сайтом:
`GroupSection` — реестр направлений сразу после первого экрана, где
ведущая колонка это КОМУ (частным лицам / разработчикам / командам /
компаниям), а не название продукта; `BusinessSection` после цены и
страница `/business` с формой заявки. Пункт «Бизнес» стоит в шапке —
компания не должна искать вход для себя, пролистывая витрину для
частных лиц. Цен направлений в реестре нет намеренно: цена подписки
живёт в `src/lib/plans.ts`, цены машин — на своих страницах, и
пятое место для той же суммы заводить нельзя.

**Акцент как цвет текста — отдельный токен.** Бренд-цвет
`--px-accent` `#FF6B45` на светлом фоне даёт 2,8:1 — ниже порога AA
даже для крупного начертания. Текст и глифы красятся
`--px-accent-text` `#C0421C` (4,8:1), заливки, границы и точки
остаются на `--px-accent`. Писать `color: var(--px-accent)` в новых
правилах нельзя: это возвращает нечитаемый текст.

**Шкала чернил проходит AA.** `--px-text-3` 0,66 и `--px-text-4` 0,60
(было 0,58 и 0,42 — 4,4:1 и 2,7:1, то есть надзаголовки и подписи
показаний не проходили порог). Ослаблять прозрачность обратно ради
воздуха нельзя: этим набраны 11-пиксельные метки.

**Шрифты.** Фирменный MTS Wide лежит в `/public/fonts` и
предзагружается в `layout.tsx`: им набран весь первый экран, и без
предзагрузки страница переверстывается при подмене шрифта (на
`/infrastructure` это давало сдвиг макета 0,23). Inter из стека убран
совсем — он был фолбэком фолбэка и стоил трёх пунктов Lighthouse
сначала блокирующей ссылкой на Google Fonts, потом отдельным
CSS-чанком через `next/font`.
...
Проверка — на клиенте и по каждому полю отдельно, ошибка живёт рядом
со своим полем (`aria-describedby`, `aria-invalid`), фокус уходит на
первое неверное. Служебные английские строки от API наружу не
показываются. Масштабирование пальцами не запрещаем: `user-scalable`
и `maximum-scale` из viewport убраны (WCAG 1.4.4).
...
Корневой `layout.tsx` держит шаблон `%s — Atlas Secure`, поэтому имя
компании в заголовок страницы не дописывается. Structured data —
одна точка на проект, `src/components/pixel/SiteJsonLd.tsx`
(Organization + WebSite + `parentOrganization: QoDev`); заводить
JSON-LD на странице нельзя, это вторая расходящаяся карточка
компании в выдаче.

**Неподтверждённые утверждения** собраны в `COMPLIANCE-CHECK.md`:
сертификации, названия чужих площадок, показатели SLA и
коммерческие условия, которых нет в коде. Новое утверждение такого
рода добавляется на страницу только вместе со строкой в этом файле.
