...
`Magnetic` (кнопка тянется к курсору), `Scramble` (текст
раскодируется из шума при появлении), `SplitText` (заголовок
набирается по буквам — разбивка двухуровневая, сначала на слова,
иначе браузер переносит строку посреди слова), `SpotlightLayer`
(световое пятно за курсором на любой карточке с классом `.px-spot` —
один делегированный слушатель на документ, а не ref в каждом
компоненте), `useTilt` (наклон плоскости), `useGridTorch` (фонарик по
пиксельной сетке первого экрана), `PixelTrail` (шлейф из пикселей за
курсором, живёт только в герое), `PixelDissolve` (содержимое
собирается из клеток), `useScrollScene` (прогресс прокрутки секции в
переменную `--p`, слои расходятся правилами CSS), `useMouseLayer`
(слой догоняет курсор с инерцией), `usePixelBurst` (вспышка пикселей из точки
нажатия — узлы создаются на время полёта и убираются по
`animationend`), `RollingNumber` (барабан разрядов). Приёмы взяты из практики наградных промо-сайтов и
переложены на пиксельный язык системы. Правила те же: только
`transform`, `opacity` и CSS-переменные, значение считает rAF,
`prefers-reduced-motion` выключает движение, а не содержание.
Дозировка: магнитная кнопка — одна на экран, раскодирование — только
на коротких служебных метках (надзаголовки секций), наклон — на
карточке-иллюстрации.
...
**Заголовок первого экрана набирается шрифтом, а не точками.**
Точечная версия хорошо смотрелась на скриншоте, но читать её
приходилось по буквам — главное сообщение сайта оказалось самым
трудным для чтения местом на экране. Приём остался там, где он
ничего не решает для понимания: декоративное слово в секции
безопасности. Масштаб держится кеглем и ступенчатыми строками;
строки заголовка не переносятся (`white-space: nowrap`), иначе
ступенчатый ритм превращается в лесенку с одиноким предлогом.

**Первый экран** держит четыре вещи и ни одной лишней: метка
состояния, заголовок во всю ширину, одна строка смысла, одно
действие и приборная полоса внизу. Лид на три строки, второй и
третий призывы и список обещаний уехали ниже по странице, где у
читателя уже есть вопрос, на который они отвечают. Кадр обязан
помещаться в экран целиком вместе с полосой показаний — высота
заголовка считается от высоты окна, а не задаётся числом.

**Нативные scroll-driven анимации.** Там, где браузер понимает
`animation-timeline: view()`, проявление секций считает он сам — без
наблюдателя и rAF; правило добавочное, в остальных браузерах
работает прежний механизм на IntersectionObserver, и содержимое
видно в любом случае.

**Залипающие сцены** — `StickyScene`: экран держит кадр, пока
читатель проходит высоту сцены, а содержимое сменяется шаг за шагом
(подключение, гарантии безопасности). Это не перехват прокрутки:
кадр держит `position: sticky`, колесо и жест работают как обычно,
скрипт лишь считает прогресс. Скорость задаётся высотой сцены
(`stepVh`, по умолчанию полтора экрана на шаг) — при одном экране
шаг проскакивал раньше, чем читатель успевал дочитать строку.

**Бегущие строки** ведёт `useMarqueeFlow`: собственный ход и толчок
от прокрутки складываются в одну позицию. CSS-анимации на дорожках
нет намеренно — две силы, писавшие в один `transform`, давали рывки
в момент прокрутки.

**Трендовый слой главной (2026/2027).** Из практики агентских и
редакционных сайтов взяты четыре приёма, и все они работают на
содержание, а не на украшение: кинетический шов между секциями
(`KineticSeam` — крупная строка фактов вместо пустой отбивки, все
числа из кода), видимый каркас колонок (`.px-frame`), моноширинные
служебные метки (номера разделов, подписи показаний — отделяют
служебный слой от содержательного) и горизонтальная лента устройств
на телефоне со scroll-snap. Скролл-джекинг не применяется намеренно:
из трендов он как раз вымывается.
...
Кабинет — рабочий экран, а не витрина: кнопки здесь 44px и по ширине
содержимого (`px-btn-sm`), а не плиты `px-btn-xl` во всю карточку;
крупный кегль отдан значению, которое человек пришёл узнать (остаток
срока, сумма), а не служебной дате. Плиты и заголовки в 48–52px уже
однажды превратили первый экран кабинета в две строки данных на
пол-экрана прокрутки.

**Устройства** — `DEVICE_LIMIT` в `src/lib/plans.ts`: одна подписка
работает на 14 устройствах. Раньше и состав тарифа, и превью
кабинета, и ответы на /pricing обещали «безлимит устройств» — это
обещание, которого продукт не выполняет; писать его руками где бы то
ни было нельзя.
