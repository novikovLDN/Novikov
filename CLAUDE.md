...
- Тёмный фон не исчез — стянут в **фокальные вставки** `.px-focal`
  (герой, финальный блок перед футером, мобильное полноэкранное меню):
  свой фон `--px-focal` `#14140F`, своя пиксельная сетка (два
  `linear-gradient` на `::before` с радиальной маской, шаг = `--px-cell`),
  свой белый текст. Любая светлая карточка внутри фокальной вставки
  обязана сама объявлять `color` — иначе наследует белый и становится
  нечитаемой на своей белой поверхности (проверенный класс регрессии).
...
- Плоские заливки запрещены: каждая поверхность получает
  альфа-градиент и кромку света `--px-edge`. Приём элевации разный по
  фону: на `.px-focal` — свет (альфа-белый, см. research/04.md), на
  светлом корпусе — обычный `box-shadow` (`--px-edge` /
  `--px-edge-strong`). Один и тот же токен-слот не переиспользуется
  между двумя смыслами без ревизии всех потребителей.
- Текст: `--px-text` `#14140F`, `--px-text-2` `#4B4A44`,
  `--px-text-3/4` — подписи и мета (внутри `.px-focal` — инвертированы
  на белый через локальные правила `.px-focal .px-text-*`).
...
Живёт только внутри фокальных вставок. Мини-версия `MiniCubes`
используется как буллет надзаголовков.
