...
export async function POST(request: NextRequest) {
  ...
  for (const row of report.rows) {
    ...
    try {
      ...
      // Re-audit this specific user to show AFTER state.
      ...
      const panelUser = nowRow?.remnawave_user_uuid ? await getUser(nowRow.remnawave_user_uuid) : null;
      const diff = diffUser(nowRow, panelUser);
