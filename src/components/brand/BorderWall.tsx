...
void main() {
  ...
  // Стена живёт и без курсора. Две медленные волны разной длины и
  // разного направления идут сквозь поле и сдвигают порог: ячейки
  // гаснут и загораются полосами, как рябь. Периоды взяты
  // несоизмеримыми, чтобы рисунок не повторялся заметным циклом.
  float wave = sin(frag.x * 0.0043 + uTime * 0.33)
             + sin(frag.y * 0.0061 - uTime * 0.21)
             + sin((frag.x + frag.y) * 0.0027 + uTime * 0.13);
  float k = uDissolve + erase * 1.15 - depth * 0.28 + wave * 0.035;
  ...
  // Редкие вспышки: у каждой ячейки своя фаза, и раз в долгое время
  // она коротко загорается кислотой. Порог узкий — в кадре горит
  // несколько ячеек из тысяч, это читается как признак жизни, а не
  // как мигание.
  float spark = smoothstep(0.9965, 1.0, fract(h * 6.37 + uTime * 0.055)) * alive;

  vec3  color = uWall * alive * breath + uAcid * (rim * 0.9 + spark * 0.7);
  float alpha = (alive * 0.92 + rim + spark * 0.6) * sq;
