 ...
 * сменяют друг друга. Вместе с ними светлеет бумага: приглушённая на
 * первом такте, обычная на втором, освещённая на третьем. Это
 * единственный переход такого рода на сайте, и он совпадает со
 * смыслом: до — закрыто, после — открыто.
 *
 * Раньше здесь корпус переворачивался из чернил в бумагу. Приём
 * работал, но требовал тёмной основы, а она в системе запрещена
 * (ТЗ 15.6). Осветление даёт то же ощущение на светлом корпусе:
 * тот же жест, другой материал.
 *
...
export default function ManifestoScene() {
  ...
  useGSAP(
    () => {
      ...
      gsap.set(node, { backgroundColor: "var(--g-paper-2)" });
      ...
      // Осветление — на последней строке, не раньше: смысл и материал
      // обязаны меняться одновременно. Промежуточный такт задаётся
      // отдельным шагом, иначе разница между «приглушённой» и
      // «освещённой» бумагой проскакивает незамеченной.
      tl.to(node, { backgroundColor: "var(--g-paper)", duration: 0.6 }, 0.9)
        .to(node, { backgroundColor: "var(--g-vellum)", duration: 0.8 }, ">-0.2");
