...
interface Platform {
  ...
  app?: string;
  span: string;
  /** Крупная ячейка bento: иконка масштабируется под её площадь. */
  featured?: boolean;
}
        ...
        <ul className="ls-stagger grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 lg:grid-rows-[repeat(2,minmax(168px,auto))] gap-3 sm:gap-4">
          {PLATFORMS.map((p) => (
            <li key={p.name} className={`ls-reveal ${p.span}`}>
              <Spotlight
                as="article"
                className="ls-card group relative overflow-hidden h-full min-h-[150px] p-5 sm:p-6 flex flex-col justify-between"
              >
                {p.featured && (
                  <span className="ls-device-art" aria-hidden>
                    <Icon name={p.icon} size={132} />
                  </span>
                )}
                <Icon
                  name={p.icon}
