    ...
    return (
      <div className="dv2-card h-[52px] w-full rounded-2xl px-4 flex items-center justify-between gap-3 opacity-50 cursor-not-allowed">
        <div className="flex items-center gap-3 min-w-0">
          <div className="h-8 w-8 rounded-xl bg-[color:var(--px-surface-2)] border border-[color:var(--px-line)] flex items-center justify-center text-[color:var(--px-text-2)] shrink-0">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
              <path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9" />
              <path d="M13.73 21a2 2 0 01-3.46 0" />
            </svg>
          </div>
          <div className="text-[13px] font-medium text-[color:var(--px-text-2)] truncate">Push не поддерживается</div>
        </div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 min-w-0">
        ...
        <div className="text-[13px] font-medium text-[color:var(--px-text)] truncate">Push-уведомления</div>
      </div>
      <div
        className={`relative w-10 h-6 rounded-full transition-colors duration-300 shrink-0 ${
          enabled ? "bg-[color:var(--px-accent)]" : "bg-[color:var(--px-surface-2)]"
        }`}
      >
        <div
